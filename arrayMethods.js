...
// CODE HERE
const totalPrice = orders.map((element) => element.price*(1+element.tax))
...
let bobOrders = purchases.filter(order => order.owner === 'Bob')

let bobTotal = bobTotal.reduce((acc, cv) => {
  return acc + cv.price
},0)